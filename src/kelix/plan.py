"""``kelix plan`` — interview the owner, then draft roadmap + backlog from a goal."""
...
def planning_phase_slug(goal: str) -> str:
    """Stable directory name under ``.kelix/phases/`` derived from the goal."""
...
class PlanRunner:
    ...
    def run(
        ...
        run_dir = cfg.kelix_dir / "runs" / f"plan-{run_id}"
        ...
        phase_dir = cfg.root / ".kelix" / "phases" / phase_slug
        ...
        if existing and questions_answered(existing):
            ...
            log(f"kelix plan {run_id}: using answered interview from {questions_path}")
        ...
        if existing and not questions_answered(existing):
            if not is_tty:
                log(
                    ...
                    "then re-run kelix plan with the same goal."
                )
            ...
            log(f"kelix plan {run_id}: interview complete, drafting plan")
            return self._run_draft(
                workdir,
                branch,
                run_dir,
        ...
        log(f"kelix plan {run_id}: branch={branch or '(in place)'} — interview")
        ...
        if not is_tty:
            ...
            log("Fill in each answer: line, then re-run kelix plan with the same goal.")
            result.status = "awaiting_answers"
            result.iteration = interview.iteration
            return result
    ...
    def _run_draft(
        ...
        started = time.monotonic()
        checkpoint(workdir, "kelix: pre-plan checkpoint")
        sha_before = head_sha(workdir)

        prompt = assemble_planning_prompt(self.cfg, goal=goal, role=PLANNING_ROLE)
        log(f"kelix plan {result.run_id}: branch={branch or '(in place)'} — draft")
        ...
        checkpoint(workdir, "kelix: post-plan auto-checkpoint")
