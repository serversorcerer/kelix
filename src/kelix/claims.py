...
def _claims_dir(kelix_dir: Path) -> Path:
    return kelix_dir / CLAIMS_SUBDIR


def _claim_path(kelix_dir: Path, task_id: str) -> Path:
    return _claims_dir(kelix_dir) / f"{task_id}.json"
...
def heartbeat(kelix_dir: Path | str, task_id: str, agent_id: str) -> bool:
    """Refresh heartbeat on an owned claim. Returns False if not owned."""
    path = _claim_path(Path(kelix_dir), task_id)
...
def release_claim(kelix_dir: Path | str, task_id: str, agent_id: str) -> bool:
...
def mark_claim_done(kelix_dir: Path | str, task_id: str, agent_id: str) -> bool:
...
def is_claimed(
    ...
    claim = _read_claim(_claim_path(Path(kelix_dir), task_id))
    if claim is None:
        return False
    return not _is_stale(claim, stale_after_s)


def list_claims(kelix_dir: Path | str) -> list[dict]:
    """Return all claim records under the fleet claims directory."""
    claims_dir = _claims_dir(Path(kelix_dir))
