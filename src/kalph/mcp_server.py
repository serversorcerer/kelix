"""Kalph as an MCP server (stdio, JSON-RPC 2.0).

Lets any MCP-capable agent (Kiro first) drive Kalph through tool calls: start a
...
TOOLS = [
    {
        "name": "kalph_run",
        "description": "Start a Kalph loop run against the repository. Runs "
    ...
    {
        "name": "kalph_memory",
        "description": "Inspect Kalph's memory: project memory, recent episode "
    ...
    {
        "name": "kalph_stop",
        "description": "Set the kill switch (.kalph/STOP) so active/future runs "
...
class MCPServer:
    ...
    def _tool_memory(self, args: dict) -> str:
        ...
        cfg = load_config(self.root)
        project = cfg.kalph_dir / "memory" / "project.md"
    ...
    def _dispatch_tool(self, name: str, args: dict) -> str:
        impl = {
            "kalph_run": self._tool_run,
            "kalph_status": self._tool_status,
            "kalph_memory": self._tool_memory,
            "kalph_stop": self._tool_stop,
        }.get(name)
        if impl is None:
    ...
    def handle(self, request: dict) -> dict | None:
        ...
        if method == "initialize":
            return self._ok(req_id, {
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "kalph", "version": __version__},
            })
