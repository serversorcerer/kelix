...
def lint_backlog(tasks: list[Task], *, scope: str = "active") -> list[Finding]:
    ...
    (only kalph-authored proposed tasks, for draft-plan validation).
    """
    ...
    lint_tasks = tasks
    if scope == "proposed":
        lint_tasks = [t for t in tasks if t.by == "kalph" and t.status == "proposed"]
...
def lint_repo(root: Path) -> list[Finding]:
    """Lint ``.kalph/backlog.md`` in a repository."""
    backlog_path = root / ".kalph" / "backlog.md"
    if not backlog_path.is_file():
        return [Finding("", "backlog_missing", "missing .kalph/backlog.md")]
...
def validate_plan(workdir: Path, base_sha: str = "") -> list[Finding]:
    ...
    Checks roadmap parse, proposed-only kalph tasks, REQ coverage, backlog
