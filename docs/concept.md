# Concept: what Kalph is and why

Kalph is the [Ralph loop](https://ghuntley.com/ralph/), rebuilt for
...
wins through repetition, not cleverness. Kalph keeps that core intact and adds
...
Kalph treats four properties of the original loop as **invariants** (full
...
*How Kalph preserves it:* the template (`.kalph/prompts/iteration.md`, or the
built-in default in `src/kalph/prompt.py`) is loaded **once per run**. Memory
...
*How Kalph preserves it:* the default adapter shells out to
...
- the completion sentinel — the agent prints `KALPH COMPLETE` when every
...
- the kill switch — `kalph stop` writes `.kalph/STOP`, and runs halt before
...
(`.kalph/backlog.md`), the memory (`.kalph/memory/`), transcripts and run
records (`.kalph/runs/<id>/`), and fleet coordination (`.kalph/fleet/`). The
...
Plain Ralph trusts the agent to say when it is finished. Kalph does not. The
...
`KALPH COMPLETE` printed while verification is red is **ignored** and counted
...
Kalph makes running them a loop responsibility rather than an agent promise.
...
bus. Kalph deliberately rejects that design.
...
- **Multi-agent chatter is a non-deterministic mess.** Kalph's fleet mode
