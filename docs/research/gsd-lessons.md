...
under `.planning/`. Studied for Kelix's planning core; sources: repo README,
...
- **Fresh context is the mechanism, not a nicety.** GSD: "an executor that
  ...
  executor." Kelix: every iteration is a fresh process. Same insight,
  arrived at independently from Ralph.
- **State lives in files, never in a conversation.** GSD's `.planning/` is
  Kelix's `.kelix/`. Both are git-versioned, human-readable, auditable.
- **Verification is a distinct step that checks intent, not just exit
  codes.** GSD's verifier checks requirement coverage; Kelix's runner
...
1. **A navigation spine (`STATE.md`).** GSD: "when any workflow starts, it
   reads STATE.md to orient itself." Kelix's fresh agent currently orients
...
2. **Top-down hierarchy: milestone → phase → task.** GSD scopes milestones
   ...
   safely executed in one loop." Kelix has only tasks; goals arrive
...
3. **Decisions captured before planning (`CONTEXT.md` / Discuss).** GSD's
   ...
   without it. Kelix equivalent: a per-phase decisions file the owner can
   fill in (or the loop proposes, owner edits) before decomposition — the
   same one-line-edit steering Kelix already favors.
4. **Requirement IDs and coverage-checked done.** GSD: "a phase is not done
   ...
   Kelix's verified-done gates each task; a phase gate should check every
   REQ-ID maps to a verified task before the phase closes.
5. **Waves for parallelism.** GSD orders plans into dependency waves so
   parallel executors touch non-overlapping concerns. Kelix's fleet claims
...
6. **A light path for light work.** GSD's `/gsd-quick` exists because "the
   phase loop is overkill for renaming a variable." Kelix's flat backlog IS
...
- **A long-lived orchestrator session.** GSD keeps a lean main session that
  ...
  rather than a file — and per the Ralph invariants, Kelix replaces it
...
- **Runtime-specific lifecycle hooks.** GSD registers per-runtime hooks
  ...
  Kelix's adapter boundary is a subprocess exit code — portable by
  construction.
- **Separate researcher/planner/plan-checker agent types in-core.** Kelix's
