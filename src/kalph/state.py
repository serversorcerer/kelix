"""Read and write `.kalph/STATE.md` — the runner-maintained navigation spine."""
...
def load_state(kalph_dir: Path | str) -> State | None:
    """Load STATE.md from *kalph_dir*. Missing file returns None."""
    path = Path(kalph_dir) / STATE_FILE
    if not path.is_file():
        return None
    return _parse_state_text(path.read_text(encoding="utf-8"))


def write_state(kalph_dir: Path | str, state: State) -> Path:
    """Write STATE.md to *kalph_dir* using the fixed schema. Returns the path."""
    path = Path(kalph_dir) / STATE_FILE
    lines = [
        H1,
        "",
